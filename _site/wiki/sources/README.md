---
type: "source"
title: "Source Notes"
description: "Source notes record evidence status and use boundaries. They are not substitutes for raw sources; they are navigable summaries that point to canonical URLs and localized files when available."
timestamp: "2026-06-26T00:00:00Z"
okf_version: "0.1"
okfr_role: "index"
okfr_summary: "Source notes record evidence status and use boundaries. They are not substitutes for raw sources; they are navigable summaries that point to canonical URLs and localized files when available."
---

# Source Notes

Source notes record evidence status and use boundaries. They are not substitutes
for raw sources; they are navigable summaries that point to canonical URLs and
localized files when available.

## Current Notes

- [Karpathy LLM-Wiki Pattern](SOURCE-KARPATHY-LLM-WIKI-2026.md)
- [Current README Inventory](SOURCE-README-ORIGINAL-2026-06-02.md)
- [Code as Agent Harness Survey](SOURCE-CODE-AS-AGENT-HARNESS-SURVEY-2026.md)
- [UNOFFICIAL-DRAFT Agentic AI Governance UK MCP](SOURCE-UNOFFICIAL-DRAFT-AGENTIC-AI-GOVERNANCE-UK-MCP-2026.md)
