---
title: "A Survey of Scientific Large Language Models: From Data Foundations to Agent Frontiers"
note_type: "paper"
status: "source-integrated"
tags: ["paper", "code-as-agent-harness"]
paper_id: "a-survey-of-scientific-large-language-models-from-data-foundations-to-ag-4493b81b"
canonical_urls:
  - "https://arxiv.org/abs/2508.21148"
local_source_paths:
  - "sources/raw/arxiv/2508.21148.pdf"
source_status: "integrated"
survey_layers:
  - "Applications and Emerging Fields"
survey_sections:
  - "Scientific Discovery Agents"
survey_subsections:
  - "Toward Agentic and Instruction-Following Science"
last_reviewed: "2026-06-02"
---

# A Survey of Scientific Large Language Models: From Data Foundations to Agent Frontiers

## Inventory Metadata

- Paper ID: `a-survey-of-scientific-large-language-models-from-data-foundations-to-ag-4493b81b`
- Venue labels: arXiv 2025
- Source status: `integrated`
- Canonical URL:
  - [https://arxiv.org/abs/2508.21148](https://arxiv.org/abs/2508.21148)

## Survey Placement

| Layer | Section | Subsection | Venue |
| --- | --- | --- | --- |
| Applications and Emerging Fields | Scientific Discovery Agents | Toward Agentic and Instruction-Following Science | arXiv 2025 |

## Localized Sources

- `sources/raw/arxiv/2508.21148.pdf` (local evidence cache; not committed by default)

### Local File Metadata

- path `sources/raw/arxiv/2508.21148.pdf`, 34967666 bytes, SHA-256 `ed2372ca354e703bb5a9732781d952bf2b5df6aad6491389c4cb1a0c6ba9b69f`

## Source-Backed Notes

- Inventory fact: this reference appears in the README paper table with the placement above.
- Source fact: a localized source file is available in this repository.
- Source fact: automated local text extraction recorded 10846 characters using `pdftotext`.
- Source cue: localized text contains harness-relevant terms: `agent` (2), `environment` (2).
- Source cue: automated topic tags: `agent`, `environment`.
- Integration note: these notes are automated extraction cues, not a human literature review.

## Cross-References

- [Paper index](README.md)
- [Taxonomy map](../maps/taxonomy-map.md)

## Gaps

- Replace automated extraction cues with human-reviewed contribution notes.
- Add concise source-backed contribution notes.
