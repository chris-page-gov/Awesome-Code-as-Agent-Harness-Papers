---
title: "NormCode: A Semi-Formal Language for Auditable AI Planning provides localized evidence about evi"
note_type: "claim-card"
status: "agent-reviewed"
tags: ["claim-card", "llm-wiki", "code-as-agent-harness"]
claim_id: "CLAIM-07C32A45BCE7"
claim_type: "evidence-practice"
paper_ids:
  - "normcode-a-semi-formal-language-for-auditable-ai-planning-96b226f7"
source_ids: []
source_refs:
  - "sources/raw/arxiv/2512.10563.pdf"
related_topics:
  - "evidence-observability-and-redress"
  - "scaling-the-harness"
evidence_locator: "sources/raw/arxiv/2512.10563.pdf; extracted sentence match for evidence-store"
extraction_method: "pdftotext"
review_status: "agent-reviewed"
creator: "tools/generate_claim_cards.py"
reviewer: "tools/review_claim_cards.py schema-traceability pass"
confidence: 0.82
volatility_flag: false
last_reviewed: "2026-06-02"
---

# CLAIM-07C32A45BCE7

## Statement

NormCode: A Semi-Formal Language for Auditable AI Planning provides localized evidence about evidence store and observability for the Code as Agent Harness wiki.

## Evidence

- Paper fragment:
  - [normcode-a-semi-formal-language-for-auditable-ai-planning-96b226f7](../papers/normcode-a-semi-formal-language-for-auditable-ai-planning-96b226f7.md)
- Source refs:
  - `sources/raw/arxiv/2512.10563.pdf`
- Locator: sources/raw/arxiv/2512.10563.pdf; extracted sentence match for evidence-store
- Matched terms: `trace`
- Evidence summary: Source sentence contains evidence store and observability evidence with matched cue(s): trace.

## Review

- Review status: `agent-reviewed`
- Creator: `tools/generate_claim_cards.py`
- Reviewer: `tools/review_claim_cards.py schema-traceability pass`
- Confidence: `0.82`
- Volatile source flag: `false`

## Topics

- [evidence-observability-and-redress](../topics/evidence-observability-and-redress.md)
- [scaling-the-harness](../topics/scaling-the-harness.md)
